...
<body>
    <form id="note_etud_oral" method="post" action="check_grades_student.php">
        ...
        <input type="text" id="nom" name="nom" value="<?= ($user['Nom_Utilisateur']) ?>" disabled>
        <br>
        ...
        <select name="liste-noms">
            <?php foreach ($listEtud as $row) { ?>
                <option value="<?= $row['Nom_Utilisateur']; ?>"><?= $row['Nom_Utilisateur']; ?></option>
            <?php } ?>
        </select>
        <br>
        ...
        foreach ($listparam as $param) {
            ...
            $count = $count + 1 ?>
            <label for="<?= $param['nom_param']; ?>"><?= $param['nom_param']; ?>:</label>
            <input type="number" id="<?= str_replace(" ", "_", $param['nom_param']); ?>" name="<?= str_replace(" ", "_", $param['nom_param']); ?>" min="0" max="<?= $param['nbpoint_param']; ?>" value="<?= $param['nbpoint_param']; ?>">
            <?= $param['nom_param']; ?>
            <br>
        <?php } ?>
        <br>
