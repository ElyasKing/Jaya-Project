...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    //On vérifie si l'étudiant n'est pas l'élève suivi
    $idUser = $_SESSION['user_id'];
    $query = 'SELECT * FROM utilisateur
    JOIN etudiant_tuteur ON utilisateur.ID_Utilisateur = etudiant_tuteur.ID_tuteur
    WHERE etudiant_tuteur.ID_tuteur = ' . $idUser . ' AND etudiant_tuteur.ID_etudiant = ' . $etud_ID;
    $result = $conn->query($query);
    $resultat = $result->fetch();

    if ($resultat != false) {
        header('Location: formSaisieNoteSoutenanceTuteur.php?status=errorTuteur');
        exit;
    }

    //On vérifie qu'il n'a pas déjà noté cet étudiant
    $query = 'SELECT * FROM utilisateur
    JOIN notes_soutenance ON utilisateur.ID_Utilisateur = notes_soutenance.ID_UtilisateurEvalue
    WHERE notes_soutenance.ID_UtilisateurEvaluateur = ' . $idUser . ' AND notes_soutenance.ID_UtilisateurEvalue = ' . $etud_ID;
    $result = $conn->query($query);
    $resultat = $result->fetch();
    ...
    if ($resultat != false) {
        header('Location: formSaisieNoteSoutenanceTuteur.php?status=error');
        exit;
    }
